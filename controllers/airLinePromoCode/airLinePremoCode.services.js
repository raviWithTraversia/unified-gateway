...
const getPromoCode = async (req, res) => {
  try {
    ...
    const companyData = await user.findById(userId);
    const companyId = companyData?.company_ID.toString();
    airlinePromoCodeData = await airLinePromo
      .find()
      .populate("supplierCode")
      .populate("airLineCode")
      .populate("fareFamily");
    const airlineCodeId = airlinePromoCodeData.filter(
      (element) => element?.companyId.toString() === companyId
    );
    const allpromoCode = airlinePromoCodeData.filter(
      (element) => element?.companyId.toString() === Config?.TMCID
    );

    const airlineCodeIdsSet = new Set(
      airlineCodeId.map((airline) => airline.airLineCode?._id)
    );

    const filteredPromoCode = allpromoCode.filter(
      (promo) => !airlineCodeIdsSet.has(promo?.airLineCode?._id)
    );

    const uniqueResult = Array.from(
      new Map(
        [...airlineCodeId, ...filteredPromoCode].map((item) => [
          item.airLineCode?._id,
          item,
        ])
      ).values()
    );
...
const deletePromoCode = async (req, res) => {
  try {
    ...
    let deletedPromoCode = await airLinePromo.findByIdAndDelete(
      airLinePromoCodeId
    );
    const userData = await user.findById(req.user._id);
    if (deletedPromoCode) {

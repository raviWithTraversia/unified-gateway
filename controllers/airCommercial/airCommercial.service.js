  ...
  try {
    ...
    if (checkDataExist) {
      ...
      if (resultAll) {
        ...
        const checkComHistory = await CommercialHistory.findOne({
          ...
          let resultAll = await CommercialHistory.findByIdAndUpdate(
            checkComHistory._id,
            {
              ...
              commercialFilterOldValue:
                checkComHistory.commercialFilterNewValue,
            },
            { new: true }
          );
        } else {
...
const getAirCommercialListByAirComId = async (req, res) => {
  try {
    ...
    const coloumnData = await AirCommercial.find({
      commercialAirPlanId: airCommercialPlanId,
    })
      .populate("carrier")
      .populate("supplier")
...
const getSingleAirComList = async (req, res) => {
  try {
    ...
    const coloumnData = await AirCommercial.find({ _id: airComId })
      .populate("carrier")
      .populate("supplier")
      .populate("source");
...
const getCommercialHistoryList = async (req, res) => {
  try {
    ...
    const coloumnData = await CommercialHistory.find({
      commercialId: commercialId,
    }).populate([
      {
        path: "newValue.AirCommertialRowMasterId",
        model: "AirCommertialColumnMaster",
      },
      {
        path: "newValue.AirCommertialColumnMasterId",
        model: "AirCommertialColumnMaster",
      },
      {
        path: "oldValue.AirCommertialRowMasterId",
        model: "AirCommertialColumnMaster",
      },
      {
        path: "oldValue.AirCommertialColumnMasterId",
        model: "AirCommertialColumnMaster",
      },

      {
        path: "commercialFilterNewValue.commercialFilterId",
        model: "AirCommercialFilter",
      },
      {
        path: "commercialFilterOldValue.commercialFilterId",
        model: "AirCommercialFilter",
      },
    ]);

...
module.exports.commonAirBookingCancellationCharge=async function(request,msg) {
  const requestBody=await createAirCancellationChargeRequestBodyForCommonAPI(request);
  // if (error) return { error };
    ...
    if (response?.errors?.length) return { error: response.errors[0] };
    // return { result: response?.data };
    return await convertResponseToAirCancellationCharge(request,response?.data?.journey,msg)
...
function convertResponseToAirCancellationCharge(input, journey,msg) {
  const adults = input.passengarList.filter((elem) => elem.PAX_TYPE === "ADT").length;
  const childs = input.passengarList.filter((elem) => elem.PAX_TYPE === "CHD").length;
  const infants = input.passengarList.filter((elem) => elem.PAX_TYPE === "INF").length;
...
itinerary.Sectors.map(sector => sector.AirlineCode))[0],
"Pnr":input.PNR,
"SplitedPnr":null,
"Fare":adultFare.totalFare + childFare.totalFare + infantFare.totalFare,
"AirlineCancellationFee": adultFare.airCancelationFee + childFare.airCancelationFee + infantFare.airCancelationFee,
"AirlineRefund": adultFare.refundAmount + childFare.refundAmount + infantFare.refundAmount,
"ServiceFee":adultFare.cancelationServiceFee+childFare.cancelationServiceFee+infantFare.cancelationServiceFee,
"RefundableAmt":(adultFare.refundAmount + childFare.refundAmount + infantFare.refundAmount)-(adultFare.cancelationServiceFee+childFare.cancelationServiceFee+infantFare.cancelationServiceFee),
"IsCanceled":false,
"IsError":false,
"Description":null,
"CancelReason":null
      ...
      j.itinerary.forEach(i => {
        i.priceBreakup.forEach(pb => {
          if (pb.passengerType === paxType) {
            ...
            const penalty = pb.airPenalty.find(p => p.type === "Refund" || p.Ref);
...
// console.log(pb)
            // Extract refund amount safely
            const serviceFee = pb.CommercialBreakup.find(p => p.CommercialType === "FixedServiceFees" || p.Ref);

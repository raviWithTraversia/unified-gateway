...
const addRegistration = async (req, res) => {
  try {
    ...
    if (!result.response && result.isSometingMissing) {
      ...
      result.response === "Invalid Mongo Object Id Id is not valid"
    ) {
...
const getAllRegistrationByCompany = async (req, res) => {
  try {
    const result = await registrationServices.getAllRegistrationByCompany(
      req,
      res
    );
    if (!result.response) {
      apiErrorres(res, result.message, ServerStatusCode.BAD_REQUEST, true);
    } else {
      apiSucessRes(
        res,
...
const updateRegistration = async (req, res) => {
  try {
    ...
    if (result.response === "Please pass valid registrationId or statusId") {
      ...
      apiErrorres(res, result, ServerStatusCode.SERVER_ERROR, true);
    }
  } catch (error) {
    apiErrorres(
      res,

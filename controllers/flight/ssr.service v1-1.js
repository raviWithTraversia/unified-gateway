...
const { getCommonSSRV1_1 } = require("../../services/common-ssr.service v1-1");

const specialServiceReqV1_1 = async (req, res) => {
  try {
    let {
      Authentication,
      TypeOfTrip,
    ...
    if (TravelType !== "International" && TravelType !== "Domestic") {
    ...
    } else if (Result?.Provider && Result?.Provider !== "Kafila") {
      try {
        const { result, error } = await getCommonSSRV1_1(req.body);
      ...
      Result = {
        ...
        IsFareUpdate: Result.IsFareUpdate ?? false,
        IsAncl: Result.IsAncl ?? false,
        Param: Result.Param ?? null,
        SelectedFlight: [Result.SelectedFlight] ?? [],
        FareBreakup: Result.FareDifference ?? null,
        GstData: Result.GstData ?? null,
        Ancl: null,
      };
      ...
      if (!result?.response?.IsSucess) {
        ...
        let AirlinesData = Result?.SelectedFlight[0]?.FCode;
...
const KafilaFun = async (
  ...
  console.log("running kafila function");
  let createTokenUrl;
  let flightSearchUrl;
  ...
  try {
    ...
    if (response.data.Status === "success") {
      ...
      console.log({ ssrResponse: fSearchApiResponse.data });

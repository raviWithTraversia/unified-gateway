...
const getAllledger = async (req, res) => {
  ...
  const ledgerDetails = await ledgerRail
    .find(filter)
...
const fetchLedgerRailReport = async (req) => {
  try {
    const { userId, agencyId, fromDate, toDate, transactionType } = req.body;
    if (!userId) return { error: "user id required" };

    const user = await User.findById(userId);
    if (!user) return { error: "user not found, invalid user id" };
    ...
    if (isTMC) {
      if (agencyId) query.companyId = new ObjectId(agencyId);
    } else query.companyId = user.company_ID;

    if (fromDate || toDate) {
      query.createdAt = {};
      if (fromDate) {
        if (!moment(fromDate, "YYYY-MM-DD", true).isValid())
          return { error: "invalid fromDate format, must be YYYY-MM-DD" };

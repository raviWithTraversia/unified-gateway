...
const lyraRedirectLink = async (req, res) => {
  ...
  try {
    ...
    const authHeader =
      "Basic " +
      Buffer.from(
        `${Config[Authentication?.CredentialType ?? "TEST"].lyraShopId}:${
          Config[Authentication?.CredentialType ?? "TEST"]?.lyraPassword
        }`
      ).toString("base64");
    var url = "";
    if (
...
const fetchChargeDetails = async (vads_charge_uuid, Mode) => {
  ...
  const authHeader =
    "Basic " +
    Buffer.from(
      `${Config[Status ?? "TEST"].lyraShopId}:${
        Config[Status ?? "TEST"]?.lyraPassword
      }`
    ).toString("base64");
  try {
    if (!vads_charge_uuid) {
